...
from data_loader_qm9_v4 import (
    get_qm9_loaders,
    denormalize_targets,
    QM9_TARGET_MEAN,
    QM9_TARGET_STD,
...
def evaluate_model(model, data_loader, device='cuda', target_indices=None):
    """
    ...
    if QM9_NORMALIZE:
        all_preds = denormalize_targets(all_preds_norm, target_indices)  # ← ADDED target_indices
        all_targets = denormalize_targets(all_targets_norm, target_indices)  # ← ADDED target_indices
    else:
...
def print_results(results, target_indices=None):
    ...
    # All properties with their indices and paper MAEs
    all_properties = [
        (0,  'α (Bohr³)',    0.050),
        (1,  'Δε (meV)',     29),
        (2,  'ε_HOMO (meV)', 14),
    ...
    # If target_indices specified, only show those properties
    if target_indices is not None:
        # Filter to only selected properties
        selected_properties = [p for p in all_properties if p[0] in target_indices]
        # Create mapping from original index to result index
        idx_map = {orig_idx: new_idx for new_idx, orig_idx in enumerate(target_indices)}
        
        for orig_idx, name, paper_mae in selected_properties:
            result_idx = idx_map[orig_idx]
            mae = results['mae_per_property'][result_idx]
            rmse = results['rmse_per_property'][result_idx]
            print(f"{name:<12} {mae:>11.6f} {rmse:>11.6f} {paper_mae:>11.6f}")
    else:
        # Show all 12 properties
        for orig_idx, name, paper_mae in all_properties:
            mae = results['mae_per_property'][orig_idx]
            rmse = results['rmse_per_property'][orig_idx]
            print(f"{name:<12} {mae:>11.6f} {rmse:>11.6f} {paper_mae:>11.6f}")
...
def main():
    ...
    parser.add_argument(
        '--checkpoint',
        type=str, 
        default='trained_models/QM9/20260206_004852/best_model_checkpoint.pt',
    ...
    parser.add_argument(
        '--save_dir',
        type=str,
        default='trained_models/QM9/test_results',
        help='Save directory'
    )
    ...
    target_indices = config.get('target_indices', None)  # Get from config
    print(f"Using target indices for denormalization: {target_indices}")
    ...
    results = evaluate_model(model, test_loader, args.device, target_indices)  # Pass it here
    
    #results = evaluate_model(model, test_loader, args.device)
    
    # Print results
    # Around line 280
    print_results(results, target_indices)  # ADD target_indices parameter

...
from NewFunctions.GATA_and_all2all.activation import GlobalNodeAttention
...
@registry.register_model("equiformer_v2_matpes_all2all")
class EquiformerV2_MatPES(BaseModel, nn.Module):
    ...
    def __init__(
        ...
        weight_init='normal',use_global_attn=True,   # ← new
        global_attn_rope=True,  # ← new
    ):
        super().__init__()
        ...
        self.global_attn_rope  = global_attn_rope
        if use_global_attn:
    ...
    def forward(self, data):
        ...
        if self.global_attn is not None:
            x_scalar = x.embedding[:, 0, :].contiguous()

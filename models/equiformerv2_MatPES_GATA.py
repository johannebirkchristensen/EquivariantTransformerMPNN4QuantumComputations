...
Changes vs original:
  - Computes rl_ij (edge spherical harmonics, l>=1) once after graph construction
    and passes it into every TransBlockV2 / SO2EquivariantGraphAttention.
  - SO2EquivariantGraphAttention now uses GATAValueActivation instead of
    SeparableS2Activation on the value path.  rl_ij and X_j (neighbour steerable
    features, extracted before rotation) are used as per-degree geometric gates.
...
from NewFunctions.Gotennet_inspired.transformer_block import FeedForwardNetwork, TransBlockV2
...
def init_edge_rot_mat(edge_distance_vec):
    """
    Deterministically pick the cardinal axis least-aligned with each edge
    to avoid the assertion failure in the baseline for axis-aligned bonds.
    Result is DETACHED — rotation frames define geometry, not the energy landscape.
    """
...
class EquiformerV2_MatPES(BaseModel, nn.Module):
    ...
    def __init__(
        ...
        # ── spherical harmonics for rl_ij computation ─────────────────────────
        # These are used in forward() to compute edge SH in the original frame.
        # We build the SphericalHarmonics operator once here (no learnable params).
        _sh_irreps   = o3.Irreps.spherical_harmonics(max(lmax_list))
        self._sphere = o3.SphericalHarmonics(
            _sh_irreps,
            normalize=False,      # we pre-normalise the vectors ourselves
            normalization='norm',
        )
        # number of SH coefficients for l>=1: (lmax+1)^2 - 1
        self._rl_ij_dim = (max(lmax_list) + 1) ** 2 - 1
    ...
    def generate_graph(self, pos, batch, cell):
        ...
        for b_idx in range(cell.shape[0]):
            ...
            # use original pos (with grad) for dvec so forces can be computed
    ...
    def _compute_rl_ij(self, edge_distance_vec: torch.Tensor) -> torch.Tensor:
        """
        Compute edge spherical harmonics for l = 1 .. lmax in the ORIGINAL
        (unrotated) coordinate frame.

        Steps:
            1. Normalise edge direction vectors to unit length.
            2. Evaluate SH up to lmax using e3nn.
            3. Drop the l=0 component (constant, carries no directional info).

        Args:
            edge_distance_vec: [E, 3]  raw displacement vectors (may have grad)

        Returns:
            rl_ij: [E, (lmax+1)^2 - 1]  SH coefficients for l >= 1, detached.
                   Detached because rl_ij defines geometry (like edge_rot_mat),
                   not the energy landscape.  Grad flows through edge_distance
                   (the radial part) instead.
        """
        # normalise — detach so this path doesn't accumulate grad
        vec_det  = edge_distance_vec.detach()
        norm     = vec_det.norm(dim=-1, keepdim=True).clamp(min=1e-8)
        unit_vec = vec_det / norm                          # [E, 3]

        # evaluate spherical harmonics  [E, (lmax+1)^2]
        sh_full = self._sphere(unit_vec)                   # [E, (lmax+1)^2]

        # drop l=0 (index 0); keep l=1 .. lmax
        rl_ij = sh_full[:, 1:]                             # [E, (lmax+1)^2 - 1]
        return rl_ij    # already detached (computed from vec_det)
    ...
    def forward(self, data):
        ...
        pos = data['pos']   # requires_grad=True during training

        edge_index, edge_distance, edge_distance_vec = self.generate_graph(
            pos, data['batch'], data['cell']
        )

        # ── edge rotation matrices (detached) ─────────────────────────────────
        edge_rot_mat = init_edge_rot_mat(edge_distance_vec)
        for i in range(self.num_resolutions):
            self.SO3_rotation[i].set_wigner(edge_rot_mat)

        # ── edge spherical harmonics, original frame (detached) ───────────────
        # Computed once and shared across all layers.  Both rl_ij and edge_rot_mat
        # are derived from the same unit direction vectors but serve different roles:
        #   edge_rot_mat → rotates irreps into the edge-aligned frame for SO2 conv
        #   rl_ij        → geometric gates in GATAValueActivation (original frame)
        ...
        for i in range(self.num_resolutions):
            x.embedding[:, offset_res, :] = self.sphere_embedding(atomic_numbers)[:, offset:offset + self.sphere_channels] \
                if self.num_resolutions > 1 else self.sphere_embedding(atomic_numbers)
            offset     += self.sphere_channels
            offset_res += int((self.lmax_list[i] + 1) ** 2)

        # ── edge features (retain grad for force computation) ─────────────────
        ...
        for i in range(self.num_layers):
            x = self.blocks[i](
                ...
                edge_index,
                batch=data['batch'],
                rl_ij=rl_ij,          # ← new: passed to GATAValueActivation
            )

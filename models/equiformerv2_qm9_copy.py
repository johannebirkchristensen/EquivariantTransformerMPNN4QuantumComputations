...
class EquiformerV2_QM9(nn.Module):
    ...
    def generate_graph(self, data):
        ...
        Args:
            data: Dictionary with 'pos', 'batch', 'atomic_numbers'
        
        Returns:
            edge_index: [2, num_edges] edge indices
            edge_distance: [num_edges] edge distances
            edge_distance_vec: [num_edges, 3] edge vectors
            cell_offsets: None (no PBC for QM9)
            _: None (placeholder for OCP compatibility)
            neighbors: [num_nodes] number of neighbors per node
        """
        pos = data['pos']
        batch = data['batch']
        
        # Build edge list based on distance cutoff
        # For each node, find neighbors within max_radius
        num_atoms = pos.shape[0]
        device = pos.device
        
        # Compute pairwise distances
        # This is inefficient but simple - for production use torch_cluster.radius_graph
        edge_index_list = []
        edge_distance_list = []
        edge_distance_vec_list = []
        
        # Get unique batch indices
        batch_indices = torch.unique(batch)
        ...
        for batch_idx in batch_indices:
            ...
            # Compute pairwise distances within this graph
            diff = pos_batch.unsqueeze(0) - pos_batch.unsqueeze(1)  # [N, N, 3]
            dist = torch.norm(diff, dim=2)  # [N, N]
            
            # Find edges within cutoff (excluding self-loops)
            within_cutoff = (dist < self.max_radius) & (dist > 0)
            src, dst = torch.where(within_cutoff)
            
            # Limit number of neighbors if needed
            if self.max_neighbors is not None and len(src) > 0:
                # For each destination node, keep only closest max_neighbors sources
                unique_dst = torch.unique(dst)
                keep_mask = torch.zeros(len(src), dtype=torch.bool, device=device)
                
                for d in unique_dst:
                    mask_d = (dst == d)
                    indices_d = torch.where(mask_d)[0]
                    distances_d = dist[src[mask_d], d]
                    
                    # Keep closest neighbors
                    if len(indices_d) > self.max_neighbors:
                        _, sorted_idx = torch.sort(distances_d)
                        keep_idx = indices_d[sorted_idx[:self.max_neighbors]]
                        keep_mask[keep_idx] = True
                    else:
                        keep_mask[indices_d] = True
                
                src = src[keep_mask]
                dst = dst[keep_mask]
            
            # Map back to global indices
            src_global = node_indices[src]
            dst_global = node_indices[dst]
            
            edge_index_list.append(torch.stack([src_global, dst_global], dim=0))
            edge_distance_list.append(dist[src, dst])
            edge_distance_vec_list.append(diff[src, dst])
        ...
        if len(edge_index_list) > 0:
            ...
            # No edges (shouldn't happen with reasonable cutoff)
        ...
        neighbors = torch.zeros(num_atoms, dtype=torch.long, device=device)

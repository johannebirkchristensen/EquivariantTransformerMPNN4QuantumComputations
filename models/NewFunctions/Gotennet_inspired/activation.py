...
class S2Activation(torch.nn.Module):
    ...
    def __init__(self, lmax, mmax):
        ...
        self.mmax = mmax
        self.act = torch.nn.SiLU()
    # The projection onto the grid of sphere. 
...
class GATAValueActivation(torch.nn.Module):
    """
    GATA-inspired replacement for SeparableS2Activation inside
    SO2EquivariantGraphAttention.

    Replaces the spherical-grid nonlinearity with per-degree learned scalar
    gates applied to two geometric terms derived from edge geometry and
    neighbor steerable features:

        For l = 0  (scalar path):
            out^(0) = SiLU(o_s)                                 [E, 1, C]

        For l >= 1 (steerable path):
            out^(l) = o_d^(l) * rl_ij^(l)                      direction term
                    + o_t^(l) * X_j^(l)                         tensor term

    where:
        o_s      [E, C]          scalar gate   (from SO2conv_1 extra m=0 output)
        o_d^(l)  [E, C]          per-degree direction gate (one per l)
        o_t^(l)  [E, C]          per-degree tensor gate    (one per l)
        rl_ij^(l)[E, 2l+1]       edge spherical harmonics, degree l (original frame)
        X_j^(l)  [E, 2l+1, C]   neighbor steerable features, degree l (original frame)

    IMPORTANT: X_j and rl_ij must be extracted BEFORE _rotate() is called on
    x_message in SO2EquivariantGraphAttention.forward(), so they are in the
    same original coordinate frame as each other.

    Args:
        lmax        (int): maximum degree of irreps
        num_channels(int): hidden_channels — width of SO2conv_1 output
    """

    def __init__(self, lmax: int, num_channels: int):
        super().__init__()
        self.lmax         = lmax
        self.num_channels = num_channels
        self.scalar_act   = nn.SiLU()

        # sizes of each degree block for l = 1 .. lmax
        # e.g. lmax=4 → [3, 5, 7, 9]
        self.degree_sizes = [2 * l + 1 for l in range(1, lmax + 1)]

    def forward(
        self,
        o_s:      torch.Tensor,      # [E, C]
        o_d_list: List[torch.Tensor],# list of lmax tensors, each [E, C]
        o_t_list: List[torch.Tensor],# list of lmax tensors, each [E, C]
        X_j:      torch.Tensor,      # [E, (L+1)^2 - 1, C]  neighbour steerable (l>=1)
        rl_ij:    torch.Tensor,      # [E, (L+1)^2 - 1]      edge SH             (l>=1)
    ) -> torch.Tensor:               # [E, (L+1)^2, C]

        # ── scalar path (l = 0) ──────────────────────────────────────────────
        out_scalar = self.scalar_act(o_s).unsqueeze(1)   # [E, 1, C]

        # ── per-degree steerable path (l = 1 .. lmax) ────────────────────────
        # split X_j and rl_ij into per-degree blocks
        X_j_by_degree   = torch.split(X_j,   self.degree_sizes, dim=1)
        rl_ij_by_degree = torch.split(rl_ij, self.degree_sizes, dim=1)

        out_degrees = []
        for l_idx in range(self.lmax):                     # l_idx = 0 means l=1
            Xj_l  = X_j_by_degree[l_idx]                  # [E, 2l+1, C]
            rl_l  = rl_ij_by_degree[l_idx].unsqueeze(-1)  # [E, 2l+1, 1]

            # gates: broadcast over the 2l+1 spatial components
            od_l  = o_d_list[l_idx].unsqueeze(1)           # [E, 1, C]
            ot_l  = o_t_list[l_idx].unsqueeze(1)           # [E, 1, C]

            dir_term    = od_l * rl_l                      # [E, 2l+1, C]
            tensor_term = ot_l * Xj_l                      # [E, 2l+1, C]

            out_degrees.append(dir_term + tensor_term)     # [E, 2l+1, C]

        out_vectors = torch.cat(out_degrees, dim=1)        # [E, (L+1)^2 - 1, C]

        # ── combine scalar and steerable ─────────────────────────────────────
        return torch.cat([out_scalar, out_vectors], dim=1) # [E, (L+1)^2, C]
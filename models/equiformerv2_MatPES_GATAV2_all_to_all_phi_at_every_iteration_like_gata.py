...
with full GATA + HTR + Global Node Attention
=============================================
...
New vs previous version:
  ...
  - GlobalNodeAttention: all-to-all node attention on the invariant scalar
    channel (l=0) after all local transformer blocks. Inspired by AllScAIP.
    Preserves equivariance — steerable channels are never touched.
...
#from NewFunctions.GATA_and_all2all.transformer_block import FeedForwardNetwork, TransBlockV2
from NewFunctions.Gotennets_GATA_phi_refined_every_layer.transformer_block import FeedForwardNetwork, TransBlockV2
from NewFunctions.GATA_and_all2all.activation import HTR, GATAValueActivation, GlobalNodeAttention
...
class EquiformerV2_MatPES(BaseModel, nn.Module):
    ...
    def __init__(
        ...
        weight_init='normal',
        use_global_attn=True, global_attn_rope=True,
    ):
        ...
        # ── global node attention (all-to-all, scalar channel only) ──────────
        # Placed after blocks and output head init so _init_weights covers it too.
        # Only touches l=0 invariant channel — equivariance fully preserved.
        if use_global_attn:
            self.global_attn = GlobalNodeAttention(
                d_model   = sphere_channels,
                num_heads = num_heads,
                dropout   = alpha_drop,
                use_rope  = global_attn_rope,
                rope_dim  = 16,
            )
        else:
    ...
    def _init_t_ij(
        self,
        x_embedding:    torch.Tensor,   # [N, (L+1)^2, sphere_channels]
        edge_dist_feat: torch.Tensor,   # [E, num_rbf]
    ...
    def generate_graph(self, pos, batch, cell):
        ...
        for b_idx in range(cell.shape[0]):
            ...
            gx, gy, gz   = torch.meshgrid(images_range, images_range, images_range, indexing='ij') # here we also keeep data about the 3x3x3 other unit cells. 
    ...
    def forward(self, data):
        ...
        # ── initialize t_ij ───────────────────────────────────────────────────
        phi_r = self.distance_expansion(edge_distance)            # [E, num_rbf]
        ...
        for i in range(self.num_layers):
            x, t_ij = self.blocks[i](
                ...
                t_ij=t_ij,
                rl_ij=rl_ij,
                phi_r=edge_dist_feat,   # ← passed every layer
            )

        # ── global node attention (all-to-all, scalar channel only) ──────────
        # Applied after all local blocks, before norm and energy head.
        # Only the l=0 invariant channel is updated — equivariance preserved.
        x.embedding = self.norm(x.embedding) # normalize embedding before to eliminate NAN
        if self.global_attn is not None:
            x_scalar = x.embedding[:, 0, :].contiguous()          # [N, sphere_C]
            x_scalar = self.global_attn(x_scalar, data['batch'], pos)
            x.embedding = x.embedding.clone()
            x.embedding[:, 0, :] = x_scalar

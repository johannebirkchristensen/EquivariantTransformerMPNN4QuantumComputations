...
The script is intentionally identical to train_MatPESv2.py in its training
logic — only the startup section differs (it loads model/optimizer/scheduler
state from the checkpoint and picks up from the next epoch).

EXIT CODE 1 DEBUGGING
---------------------
If the previous run exited with code 1 (not 140 = wall-time), check:
    cat trained_models/MatPES/<run_dir>/<jobid>.err
Common causes:
  - OOM mid-epoch (rare on 80 GB but possible with large structures)
  - NaN loss (exploding gradients — increase gradient_clip_norm or reduce lr)
  - DataLoader worker crash (check for pymatgen warnings in .err)
"""
...
DEFAULT_CHECKPOINT = (
    ...
    'trained_models/MatPES/matpes_20260226_075919/'   # ← your run dir
    'best_model_checkpoint.pt'                         # ← or checkpoint_epoch_004.pt
)
# ─────────────────────────────────────────────────────────────────────────────


# ── Helpers (identical to train_MatPESv2.py) ──────────────────────────────────
...
def main():
    ...
    # Restore config from checkpoint so architecture matches exactly
    ...
    # Use energy stats from checkpoint — NOT recomputed from data
    # (recomputing gives same values but ensures consistency if data order shifts)
    ...
    # Scheduler state: load if compatible, otherwise fast-forward by stepping.
    # A KeyError on '_schedulers' means the checkpoint was saved with a plain
    # CosineAnnealingLR but we reconstruct a SequentialLR here. In that case
    # we fast-forward by calling step() for each already-completed epoch.
    ...
    if sched_state is not None:
        try:
            ...
            print(f"  Fast-forwarding scheduler to epoch {start_epoch - 1} instead.")

    if not loaded_sched:
        for _ in range(start_epoch - 1):
            scheduler.step()
        current_lr = optimizer.param_groups[0]['lr']
        print(f"  Scheduler fast-forwarded to epoch {start_epoch - 1} — LR = {current_lr:.4e}")
    ...
    def _sigterm_handler(signum, frame):
        ...
        save_checkpoint(path, make_ckpt(
            ep, _state['model'], _state['optimizer'], _state['scheduler'],
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        if save_interval and epoch % save_interval == 0:
            ...
            # Keep only two most recent periodic checkpoints
            all_ckpts = sorted([

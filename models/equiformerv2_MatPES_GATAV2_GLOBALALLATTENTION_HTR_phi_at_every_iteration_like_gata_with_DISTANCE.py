...
from NewFunctions.GATA_and_all2all.activation import HTR, GATAValueActivation, GlobalNodeAttention,GlobalNodeAttentionHTR ,GlobalNodeAttentionFullEquivariant, GlobalNodeAttentionHTR_with_distance, GlobalNodeAttentionHTR_with_ROPE
...
class EquiformerV2_MatPES(BaseModel, nn.Module):
    ...
    def __init__(
        ...
        if use_global_attn:
            self.global_attn = GlobalNodeAttentionHTR_with_ROPE(
                sphere_channels = sphere_channels,   # 128
                lmax            = max(lmax_list),    # 4
    ...
    def forward(self, data):
        ...
        # ── global node attention (all-to-all) ──────────

        x.embedding = self.norm(x.embedding) # normalize embedding before to eliminate NAN
        x.embedding = self.global_attn(x.embedding, data['batch'], pos)

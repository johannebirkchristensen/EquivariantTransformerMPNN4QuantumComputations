#!/usr/bin/env python3
"""
train_MatPESv2_original_wandb.py
==================================
Original EquiformerV2 training script (no GATA/HTR) with W&B logging added.

W&B additions vs the baseline original:
  - wandb.init() before the run-dir block
  - wandb.log() every 50 steps (loss, lr)
  - wandb.log() per epoch (train/val metrics)
  - wandb.log() for dataset stats and model param count
  - wandb.Artifact upload when best model is saved
  - wandb.log() + wandb.finish() in SIGTERM handler
  - run id saved in checkpoint so resume re-attaches to the same W&B run

Everything else (scheduler, loss weights, NaN handling) is UNCHANGED from
the original — this is intentional so you can do a fair comparison against
the GATA run in W&B.
"""
...
from configs.MatPES.config_cosinelearning import config
...
def make_ckpt(epoch, model, optimizer, scheduler, best_e_mae,
    ...
    return {
        ...
        'config':               config,
        'wandb_run_id':         wandb_run_id,   # ← so resume re-attaches to same W&B run
    }
...
def train_step(model, batch, criterion, w_energy, w_force, grad_clip, optimizer):
    ...
    NOTE: No NaN guard here — this is the original behaviour.
    If you see NaN explosions, use the GATA training script instead.
    """
    optimizer.zero_grad()
...
def main():
    ...
    w_force        = config.get('force_loss_weight',  1.0)   # original default
    batch_size     = config.get('batch_size', 32)
    total_epochs   = config.get('epochs', 200)
    base_save_dir  = config.get('save_dir')
    grad_clip      = config.get('gradient_clip_norm', 5.0)   # original default
    save_interval  = config.get('save_interval', 1)
    resume_path    = config.get('resume_from', None)

    # ── W&B init ──────────────────────────────────────────────────
    # Restore the run id from the checkpoint (if resuming) so that W&B
    # continues the same run rather than creating a new one.
    wandb_run_id = config.get('wandb_run_id', None)

    # Try to read run id from the checkpoint directly (more reliable than config)
    if resume_path and os.path.exists(resume_path):
        try:
            _peek = torch.load(resume_path, map_location='cpu')
            wandb_run_id = _peek.get('wandb_run_id', wandb_run_id)
            del _peek
        except Exception:
            pass
    ...
    run = wandb.init(
        ...
        config=config,
        name=config.get('wandb_run_name', 'equiformerv2-original-baseline'),
        tags=['equiformerv2', 'original', 'matpes-pbe', 'baseline'],
        id=wandb_run_id,
        resume='allow',
    )
    # Expose the live run id so make_ckpt can save it
    wandb_run_id = run.id
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        run_dir = os.path.join(base_save_dir, f"matpes_orig_{run_start.strftime('%Y%m%d_%H%M%S')}")
    ...
    # ── Optimizer & scheduler ─────────────────────────────────────
    # Original: plain cosine, no warmup, higher lr
    optimizer = AdamW(
        model.parameters(),
    ...
    def _sigterm_handler(signum, frame):
        ...
        save_checkpoint(path, make_ckpt(
            ...
            wandb_run_id=_emergency_state['wandb_run_id'],
        ))
    ...
    # ── global step counter (W&B x-axis, survives resume) ────────
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for batch in pbar:
            ...
            # per-step W&B log (every 50 steps)
            if global_step % 50 == 0:
                wandb.log({
                    'train/loss_step':   tot,
        ...
        # original: scheduler steps once per epoch
        scheduler.step()
